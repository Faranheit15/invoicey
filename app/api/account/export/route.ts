 ...
 *
 * Everything the app holds on the caller, in one file they can read: profile,
 * every invoice INCLUDING the ones they deleted, their feedback, and their
 * activity. The dashboard's `is_deleted: { $ne: true }` filter is deliberately
 * absent — a soft-deleted invoice is still their data, and an export that
 * hides it is not a complete answer to the request.
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [profile, invoicesRaw, feedbackRaw, activityRaw] = await Promise.all([
