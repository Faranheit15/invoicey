...
export default function InvoiceModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/70 backdrop-blur-[2px] sm:p-6">
      <div className="relative w-full max-w-6xl max-h-[95vh] overflow-hidden border shadow-2xl rounded-2xl border-slate-200 bg-slate-50">
        <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-4 border-b border-slate-200 bg-white">
          <div>
            <p className="text-xs tracking-wider uppercase text-slate-500">Invoice</p>
        ...
        <div className="max-h-[calc(95vh-80px)] overflow-auto p-4 sm:p-6">
          <article className="mx-auto overflow-hidden bg-white border shadow-md max-w-4xl rounded-xl border-slate-200">
            <header className="flex flex-wrap justify-between gap-5 px-6 py-6 border-b border-slate-200 bg-slate-900 text-slate-100 sm:px-8">
              <div>
            ...
            <section className="grid gap-4 px-6 py-5 border-b border-slate-200 sm:grid-cols-2 sm:px-8">
              <div className="p-4 border rounded-lg border-slate-200 bg-slate-50">
                <p className="text-xs font-semibold tracking-wider uppercase text-slate-500">
                  Bill From
                </p>
                <p className="mt-1 text-sm font-semibold text-slate-900">{invoice.companyName}</p>
                ...
                {invoice.companyEmail ? (
                  <p className="mt-1 text-sm text-slate-600">{invoice.companyEmail}</p>
                ) : null}
                {invoice.companyPhone ? (
                  <p className="text-sm text-slate-600">{invoice.companyPhone}</p>
                ) : null}
              </div>
              <div className="p-4 border rounded-lg border-slate-200 bg-slate-50">
                <p className="text-xs font-semibold tracking-wider uppercase text-slate-500">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-900">{invoice.billTo}</p>
                ...
                {invoice.billToEmail ? (
                  <p className="mt-1 text-sm text-slate-600">{invoice.billToEmail}</p>
                ) : null}
              </div>
            </section>
            ...
            <section className="px-6 py-5 sm:px-8">
              <div className="overflow-x-auto border rounded-lg border-slate-200">
                <div className="grid min-w-[640px] grid-cols-[64px_1.5fr_96px_140px_140px] bg-slate-100 px-4 py-2 text-xs font-semibold tracking-wider uppercase text-slate-500">
                  <div>#</div>
                  <div>Description</div>
            ...
            <section className="flex justify-end px-6 pb-6 sm:px-8">
              <div className="w-full max-w-sm text-sm">
                ...
                <div className="flex justify-between pt-3 mt-2 text-base font-semibold border-t border-slate-200 text-slate-900">
                  <span>Total</span>
            ...
            {(invoice.terms || invoice.paymentInfo || invoice.notes) && (
              <section className="grid gap-4 px-6 py-5 border-t border-slate-200 sm:grid-cols-3 sm:px-8">
                {invoice.terms ? (
                  <div>
